...
<%@ page import="adapters.psfin.beans.*" %>
<%@ page import="adapters.psfin.wf.*" %>
<%@ page import="java.io.*" %>
...
<%  
    ...
    try{
        ...
        XMLRequestBean requestXML=new XMLRequestBean(xmlBuff.toString());
        ...
        if(req.equalsIgnoreCase("GetNextImage")){
          ...
          try{
            GetVoucherFromQueue nextImage=new GetVoucherFromQueue(occi);
            docId=nextImage.getVoucher(PSFINAdapterConstant.ITYPE,aclName);
            try{
              ObtainLink link = new ObtainLink(application,userId,password,docId);
              docLink=link.getLink(request.getRequestURI(),"adapters/psfin/psfin_gateway.jsp");
            }catch(Exception e){
              logger.error(e.toString());
              errorString=e.toString().trim();
              ResetVouchers voucherReset = new ResetVouchers(occi);
              voucherReset.reset(PSFINAdapterConstant.ITYPE,aclName,new String[] {docId});
            }
          }catch(Exception e){
            logger.error(e.toString());
            errorString=e.toString().trim();           
          ...
          %>
           <jsp:forward page="psfin_response.jsp" />
          <%
        }
          ...
          %>
          <jsp:forward page="/renameVoucherAction.do" />
          <%
          ...
          DeleteVoucher deleteVoucher= new DeleteVoucher(occi,null,null,docIdsinLong,aclName,application);
          ...
          request.setAttribute("voucherId",voucherId);
          %>
          <jsp:forward page="/replaceVoucherB4Action.do" />
          <%
        }
    }catch(Exception e){
      logger.error(e.toString());
