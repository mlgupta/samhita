...
{ 
  ...
  if (!(folder.isOpen4List)){                      
   ...
   // window.location.replace("folderDocFolderClickAction.do?currentFolderId="+ folderId  );		 //Added By Sudheer  
    window.location.replace("theme_preview_treeview.jsp");
  }
...
function highlightObjLink(nodeObj) {
  ...
  if (browserVersion == 1 || browserVersion == 3) {
    ...
    if (clickedDOMObj != null) {
        ...
        clickedDOMObj.className="bgClrLvl_2";
...
<%
String treeIconPath=(String)((session.getAttribute("treeIconPath")!=null)?(session.getAttribute("treeIconPath")):("themes/standard/"));
...
HIGHLIGHT = 1
PERSERVESTATE = 1
ICONPATH ="<%=treeIconPath%>"
foldersTree = gFld('<bean:message key="theme.preview.DbSentry" />');
foldersTree.treeID = "preview";
foldersTree.xID = "DbSentry";
foldersTree.isChildExist = true;
foldersTree.isNextLevel = true;

objHome= gFld('<bean:message key="theme.preview.Home" />');
objHome.xID="Home";
objHome.isChildExist=true;
objHome.isNextLevel=true;
insFld(foldersTree,objHome);

objManish= gFld('<bean:message key="theme.preview.Manish" />');
objManish.xID="Manish";
objManish.isChildExist=false;
objManish.isNextLevel=false;
insFld(objHome,objManish);

objRajan= gFld('<bean:message key="theme.preview.Rajan" />');
objRajan.xID="Rajan";
objRajan.isChildExist=false;
objRajan.isNextLevel=false;
insFld(objHome,objRajan);

objPanditji= gFld('<bean:message key="theme.preview.Panditji" />');
objPanditji.xID="Panditji";
objPanditji.isChildExist=false;
objPanditji.isNextLevel=false;
insFld(objHome,objPanditji);

objDada= gFld('<bean:message key="theme.preview.Dada" />');
objDada.xID="Dada";
objDada.isChildExist=false;
objDada.isNextLevel=false;
insFld(objHome,objDada);

objJeet= gFld('<bean:message key="theme.preview.Jeet" />');
objJeet.xID="Jeet";
objJeet.isChildExist=false;
objJeet.isNextLevel=false;
insFld(objHome,objJeet);

objSudhi= gFld('<bean:message key="theme.preview.Sudhi" />');
objSudhi.xID="Sudhi";
objSudhi.isChildExist=false;
objSudhi.isNextLevel=false;
insFld(objHome,objSudhi);
</script>
</head>
<body class="imgData bgClrLvl_4" >
<script>
  initializeDocument();
</script>
</div>
